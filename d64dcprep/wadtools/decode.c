...
int main(int argc, char **argv) {
	...
	outlen = decodedsize(buffer);
	decode(buffer, decodebuf);
	printf("%s input size %d output size %d\n", fn, fileLen, outlen);
//	printf("%d\n", outlen);

FILE *encfile;
char encfn[256];
sprintf(encfn, "%s.dec", fn);
encfile = fopen(encfn, "wb");
fwrite(decodebuf, outlen, 1, encfile);
fclose(encfile);
//free(outbuf);
